...
class SongResource(Resource):
    ...
    def get(self, song_id=None):
        if song_id:
            ...
            return {'songs': list(map(lambda x: x.to_dict(), Song.query.all()))}
...
class PlaylistResource(Resource):
    ...
    def get(self, playlist_id=None):
        if playlist_id:
            ...
            return {'playlists': list(map(lambda x: x.to_dict(), Playlist.query.all()))}

    def post(self):
        data = PlaylistResource.parser.parse_args()
        playlist = Playlist(name=data['name'], user_id=data['creator_id'])
    ...
    def put(self, playlist_id):
        ...
        if playlist:
            ...
            playlist.user_id = data['creator_id']
            db.session.commit()
            return playlist.to_dict()
        else:
            return {'message': 'Playlist not found'}, 404
